...
export default function TechStackSection() {
  ...
  const toolsRow = [
    ...
    { name: "SigNoz", category: "Observability" },
    { name: "OpenTelemetry", category: "Telemetry SDK" },
    { name: "gRPC", category: "High-Speed RPC" },
  ...
  const languagesRow = [
    ...
    { name: "JavaScript", category: "Web Standard" },
    { name: "TypeScript", category: "Typed JS" },
    { name: "Go", category: "Systems & Concurrency" },
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Decelerate back to normal speed (1x) when scrolling stops
    ...
    >
      ...
      <div style={{ maxWidth: '1180px', margin: '0 auto 2.5rem auto', padding: '0 1.5rem' }}>
        ...
        <p style={{ color: 'var(--text-secondary, #94a3b8)', fontSize: '1.05rem', marginTop: '0.4rem', maxWidth: '650px' }}>
          Bi-directional infinite animated train featuring 2D vector brand logos in rounded square cards. Scroll faster to accelerate the train!
        </p>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
        ...
        <div style={{ overflow: 'hidden', width: '100%', position: 'relative' }}>
          {/* Gradient Edge Blurs */}
          <div style={{ position: 'absolute', top: 0, bottom: 0, left: 0, width: '100px', zIndex: 10, background: 'linear-gradient(to right, var(--bg-primary, #0b0f19), transparent)', pointerEvents: 'none' }} />
          <div style={{ position: 'absolute', top: 0, bottom: 0, right: 0, width: '100px', zIndex: 10, background: 'linear-gradient(to left, var(--bg-primary, #0b0f19), transparent)', pointerEvents: 'none' }} />
          ...
          >
            {topMarqueeItems.map((item, idx) => (
              <div
                ...
                className="rounded-square-card"
              >
                {/* 2D Vector Logo inside 84x84 Rounded Square Badge */}
                <div
                  style={{
                    width: '48px',
                    height: '48px',
                ...
                >
                  {render2DIcon(item.name)}
                </div>

                <div>
                  <div style={{ fontSize: '0.95rem', fontWeight: 800, color: 'var(--text-primary, #ffffff)' }}>
                    {item.name}
                  </div>
